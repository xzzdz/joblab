...
> อยากรู้ว่าทำถึงไหนแล้ว → [PROGRESS.md](PROGRESS.md) · อยากรู้กฎการเขียน → [../CLAUDE.md](../CLAUDE.md)
...
### ก่อนบอกว่า "เสร็จแล้ว" — ต้องผ่านทั้ง 3 อัน
...
## 6. ทำไมไฟล์ resume ไม่อยู่ใน `public/`

`public/` = ใครก็เปิดได้ ไม่ต้องล็อกอิน แถม Google เก็บเข้า index ได้ด้วย
แต่ resume มีเบอร์โทร ที่อยู่ ประวัติการทำงาน — เป็นข้อมูลส่วนบุคคลของคนอื่น

โปรเจคนี้จึงเก็บไว้ที่ `.uploads/` (นอก `public/` และถูก gitignore) แล้วให้เข้าถึงผ่าน
[`/api/resumes/[applicationId]`](../src/app/api/resumes/%5BapplicationId%5D/route.ts) ซึ่งตรวจสิทธิ์ทุกครั้ง

**ชื่อไฟล์บนดิสก์เป็น UUID ที่เราสุ่มเอง ไม่ใช่ชื่อที่ผู้ใช้ตั้ง** — เพราะถ้ามีคนตั้งชื่อไฟล์ว่า
`../../.env` แล้วเราเอาไปต่อ path ตรง ๆ จะเขียนทับไฟล์นอกโฟลเดอร์ได้ (ช่องโหว่ path traversal)

> วิธีนี้ใช้บน Vercel ไม่ได้ เพราะ filesystem หายทุกครั้งที่ deploy
> แต่โค้ดที่เหลือเรียกผ่าน `storage.ts` เท่านั้น วันย้ายไป S3 จะแก้แค่ไฟล์เดียว (Phase 6)
...
## 9. อยากเพิ่มฟีเจอร์ใหม่ ทำตามลำดับนี้
...
6. **ตรวจ** `lint` → `typecheck` → `build` → **ยิง request จริงดู status code**
7. **อัปเดต [PROGRESS.md](PROGRESS.md)** แล้ว commit
