...
### ก่อนบอกว่า "เสร็จแล้ว" — ต้องผ่านทั้ง 5 อัน
...
| `npm run test:e2e` | กดปุ่มจริงในเบราว์เซอร์จริง (Playwright) |
| `npm run build` | build จริงพังไหม + **ดูตาราง Route ท้ายผลลัพธ์ด้วย** |

ทั้ง 5 อันนี้ถูกรันอัตโนมัติทุกครั้งที่ push ด้วย [GitHub Actions](../.github/workflows/ci.yml)
...
| `saved-jobs.ts` | query งานที่บันทึกไว้ | ฝัง `userId` ใน where เสมอ |
| `password-reset.ts` | ตรรกะตั้งรหัสผ่านใหม่ | เก็บ token เป็นแฮช ใช้ครั้งเดียว |
| `email.ts` | ส่งอีเมล | ไม่มี key ก็พิมพ์ลง console แทน |
| `site.ts` | URL และชื่อเว็บ | ใช้ร่วมกับ sitemap / metadata |
...
แนวทางคือ **Swiss Editorial** — หมึกบนกระดาษ + สัญญาณสีเดียว
ลำดับความสำคัญสร้างจาก **ขนาดและน้ำหนักตัวอักษร** ไม่ใช่จากการไล่เฉดเทาหลายระดับ

### กฎ 3 ข้อของธีมนี้

1. **ห้ามเขียนสีดิบ ๆ ในคอมโพเนนต์** ใช้ token เท่านั้น (`bg-surface`, `text-ink-muted`, `border-line`)
   เพราะ token เปลี่ยนค่าเองตามโหมดสว่าง/มืด ส่วน `bg-white` ไม่เปลี่ยน แล้วโหมดมืดจะพัง
2. **สีส้มใช้เท่าที่จำเป็น** ถ้าใช้ทุกที่มันจะไม่เหลือความหมาย จุดที่ใช้ได้คือ
   ป้ายกำกับสำคัญ · ลิงก์ · สถานะ hover · ปุ่มหลักหนึ่งปุ่มต่อหน้า
3. **ตัวเลขและป้ายกำกับใช้ฟอนต์ mono** (`.label-mono` หรือ `font-mono tabular-nums`)
   เป็นลายเซ็นของธีม และทำให้ตัวเลขเรียงตรงกันเป็นคอลัมน์

### token ทั้งหมดอยู่ใน [globals.css](../src/app/globals.css)

| token | ใช้ตอนไหน |
|---|---|
| `paper` / `surface` / `surface-2` | พื้นหน้า / พื้นการ์ด / พื้นรอง |
| `ink` / `ink-muted` | ข้อความหลัก / ข้อความรอง |
| `line` / `line-strong` | เส้นคั่นบาง / ขอบช่องกรอกที่ต้องมองเห็นชัด |
| `accent` / `accent-hover` / `accent-on` / `accent-soft` | สีสัญญาณและตัวอักษรบนพื้นสัญญาณ |
| `positive` / `critical` / `warning-*` | สถานะสำเร็จ / ผิดพลาด / เตือน |

### ทุกสีผ่านการคำนวณ contrast ไม่ได้เลือกด้วยสายตา

ระบบดีไซน์แนะนำสีส้ม `#EA580C` มาตอนแรก แต่คำนวณตามสูตร WCAG แล้วได้ **3.56:1**
บนพื้นขาว ซึ่งไม่ถึงเกณฑ์ 4.5:1 ของข้อความปกติ จึงเลื่อนลงมาเป็น `#C2410C` (**5.18:1**)

ในโหมดมืดใช้ `#FB923C` (**8.73:1**) — ไม่ใช่แค่กลับขาวเป็นดำ
เพราะสีส้มเข้มเดิมบนพื้นมืดจะจมจนอ่านไม่ออก

> ตอนจะเพิ่มสีใหม่ ให้คำนวณก่อนเสมอ: ข้อความปกติต้อง ≥ 4.5:1 · ขอบและองค์ประกอบ UI ต้อง ≥ 3:1

### โหมดมืดทำงานยังไง

```
สคริปต์ใน <head>  →  อ่าน localStorage / การตั้งค่าระบบ
                     แล้วตั้ง data-theme บน <html> ก่อนหน้าถูกวาด
        ↓
CSS               →  [data-theme="dark"] เขียนทับค่า token
        ↓
ปุ่มสลับ           →  เปลี่ยน attribute + จำค่าไว้ (ไม่มี state ใน React เลย)
```

**ทำไมสคริปต์ต้องอยู่ใน `<head>` ไม่ใช่ `useEffect`:** `useEffect` ทำงานหลังเบราว์เซอร์วาดหน้าไปแล้ว
คนที่ตั้งโหมดมืดไว้จะเห็นหน้าขาวแวบหนึ่งก่อน (flash of wrong theme) ซึ่งแสบตามากตอนกลางคืน

**ทำไมปุ่มสลับไม่เก็บ state:** ถ้าเก็บ ต้องอ่านค่าจาก DOM ใน `useEffect` เพื่อให้ตรงกับของจริง
ทำให้ render รอบสองทุกครั้งที่โหลดหน้า (ESLint ของ React จับข้อนี้) และเสี่ยง hydration mismatch
วิธีที่ใช้คือแสดงไอคอนทั้งสองอันไว้ แล้วให้ CSS เลือกว่าจะโชว์อันไหน

### รายการตรวจก่อนส่งงาน UI

- [ ] ใช้ token ไม่มีสีดิบ
- [ ] ปุ่มและลิงก์ที่กดได้สูงอย่างน้อย 44px (นิ้วกดได้แม่น)
- [ ] `cursor-pointer` บนทุกอย่างที่กดได้
- [ ] transition 150–300ms ไม่ใช่เปลี่ยนทันที
- [ ] ไอคอนเป็น SVG **ห้ามใช้ emoji** (หน้าตาต่างกันทุกระบบและเปลี่ยนสีตามธีมไม่ได้)
- [ ] ปุ่มที่มีแต่ไอคอนต้องมี `aria-label`
- [ ] ทดสอบทั้งโหมดสว่างและมืด
- [ ] กด Tab ไล่ทั้งหน้าแล้วเห็นเส้นโฟกัสทุกจุด

---

## 10. เทสต์ — เทสต์อะไร และทำไมเทสต์แบบนั้น

```bash
npm run test          # Vitest — รันครั้งเดียว
npm run test:watch    # Vitest — รันใหม่อัตโนมัติตอนแก้โค้ด
npm run test:e2e      # Playwright — เปิดเบราว์เซอร์จริงแล้วกดปุ่มจริง
npm run test:e2e:ui   # Playwright แบบมีหน้าจอให้ดูทีละขั้น (ใช้ตอนไล่หาสาเหตุ)
```

แบ่งเป็น 3 กลุ่มด้วยเหตุผลต่างกัน:
...
เทสต์กลุ่มนี้สร้างข้อมูลของตัวเองด้วย prefix เฉพาะ แล้วลบทิ้งเมื่อจบ
จึงไม่แตะข้อมูล seed และรันซ้ำได้เรื่อย ๆ

> **กฎเหล็ก: เทสต์ที่ต่อ DB จริงต้องกรองเฉพาะข้อมูลของตัวเองเสมอ**
> เคยเขียน `findFirst` ลอย ๆ แล้วเทสต์ล้ม เพราะ E2E ที่รันก่อนหน้าทิ้งแถวไว้ในตารางเดียวกัน
> อย่าสมมติว่าตารางว่าง — มันไม่เคยว่างจริงเมื่อรันร่วมกับเทสต์อื่น

**กลุ่มที่ 3 — กดปุ่มจริงในเบราว์เซอร์** (`e2e/*.spec.ts`)

จำเป็นเพราะมีของที่ยิง HTTP ทดสอบไม่ได้เลย: **ปุ่มที่เป็น Client Component
เรียก Server Action ผ่าน `useTransition` ไม่ใช่ `<form>`** จึงไม่มี endpoint ให้ยิง

> เคยพยายาม replay ผ่าน HTTP แล้วพบว่า **เทสต์ผ่านทั้งที่ไม่ได้ทดสอบอะไรเลย**
> — ไป scrape เอา action ของปุ่มออกจากระบบที่อยู่บนหน้าเดียวกันมาแทน
> **เทสต์ที่ผ่านแบบผิด ๆ แย่กว่าไม่มีเทสต์**

ข้อควรระวังตอนเขียน E2E ที่เจอมาแล้ว:
- **อย่าใช้อีเมลภาษาไทย** ใน `<input type="email">` — Chrome ปฏิเสธตั้งแต่ก่อนส่งฟอร์ม
- **ปิด validation ของเบราว์เซอร์ไม่ได้** — React เขียน attribute กลับทันทีที่ re-render
  ถ้าจะทดสอบ validation ฝั่ง server ให้ทดสอบ Zod schema ตรง ๆ แทน
- **rate limit นับข้ามเทสต์** ถ้ามี dev server ตัวเก่าค้างอยู่ ตัวนับจะไม่ถูกล้าง

---

## 11. อยากเพิ่มฟีเจอร์ใหม่ ทำตามลำดับนี้
